...
-- DimWeer: 1 rij per dag voor de regio Zuid-Limburg (geen locatie-koppeling nodig,
-- alle objecten van GoedWonen Zuid liggen in dezelfde regio)
CREATE OR ALTER PROCEDURE dbo.usp_Load_DimWeer
AS
BEGIN
    TRUNCATE TABLE dbo.DimWeer;
    INSERT INTO dbo.DimWeer
        (DatumKey, GemTemperatuurC, MinTemperatuurC, MaxTemperatuurC,
    ...
    SELECT
        ...
        w.WEERTYPE
    FROM stg.WEER w
    JOIN dbo.DimDatum d ON d.Datum = w.DATUM;
END
GO
...
BEGIN
    ...
    LEFT JOIN dbo.DimWeer weer    ON weer.DatumKey = dMeld.DatumKey;
END
GO
